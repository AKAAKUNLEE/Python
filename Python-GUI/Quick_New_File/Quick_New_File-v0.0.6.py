...
def select_file_path():
    file_path = filedialog.asksaveasfilename(defaultextension='.' + extension_var.get(), filetypes=[(f"{extension_var.get()} files", f"*.{extension_var.get()}")])
    if file_path:
        entry.delete(0, tk.END)
...
# 创建选择文件路径按钮
select_file_path_button = tk.Button(root, text="选择文件路径", command=select_file_path)
select_file_path_button.pack(pady=5)
